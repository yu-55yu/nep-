...
void ProcessFFTData(uint8_t i, float phase)
{
    ...
    if(i==0)
    {
        correction = a[0]*Wave[0].BaseIdx + a[1];
    }
    else if(i==1)
    {
        correction = b[0]*Wave[1].BaseIdx + b[1];
        correction = correction+y[0]+y[1]*Wave[0].BaseIdx+y[2]*Wave[1].BaseIdx;
    }
    phase += correction; 
...
//     //             for(int k = harm_idx-bw; k <= harm_idx+bw; k++) {
//     //                 // 直接复制，不改变相位
//     //                 fft_data[2*k] = FFT_Data[2*k];
//     //                 fft_data[2*k+1] = FFT_Data[2*k+1];
//     //                 // 负频率分量
//     //                 int mirror = FFT_LEN - k;
//     //                 if(mirror >= 0 && mirror < FFT_LEN) {
//     //                     fft_data[2*mirror] = FFT_Data[2*mirror];
//     //                     fft_data[2*mirror+1] = FFT_Data[2*mirror+1];
//     //                 }
//     //             }
//     //         }
//     //     }
...
void GetPhase(void)
{
    ...
    {
        int idx = Wave[i].BaseIdx;
        Wave[i].Phase = atan2f(FFT_Data[idx* 2 + 1], FFT_Data[idx * 2]);
    }
}
